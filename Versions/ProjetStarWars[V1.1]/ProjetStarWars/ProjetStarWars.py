...
class indiv():
    ...
    def selection(pop, hcount, lcount):
        ...
        return (top + bottom)
    def __str__(self):
        return f"val: {self.val} || erreur: {self.erreur}"
...
if debug == True:
    ...
    pop = indiv.createRandPop(5)
    for i in pop:
        i.fitness(data)
    evalu = indiv.evaluate(pop)
else:
    ficname = r"position_sample.csv"
    data = LoadData(ficname)
    ...
    while solutionFound == False:
        ...
        if (evaluation[0].erreur) < 20:
        #if nbIter >= 5000:
            solutionFound = True
            res = deepcopy(evaluation[0])
        else:
            select = indiv.selection(evaluation, 10, 4) 
            cross = []
            muts = []
